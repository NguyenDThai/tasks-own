    ...
    <header className="sticky top-0 z-40 w-full backdrop-blur-md bg-white/75 dark:bg-zinc-950/75 border-b border-zinc-200 dark:border-zinc-800">
      <div className="container mx-auto px-3 sm:px-4 h-14 sm:h-16 flex items-center justify-between gap-2">
        ...
        <div className="flex items-center gap-1.5 sm:gap-3 min-w-0">
          {user && (
            <div className="flex items-center gap-2 sm:gap-4 mr-1 sm:mr-2 pr-2 sm:pr-4 border-r border-zinc-200 dark:border-zinc-800 min-w-0">
              {/* User info */}
              <div className="flex items-center gap-1.5 text-sm font-medium text-zinc-600 dark:text-zinc-400 min-w-0">
                ...
                <span className="truncate max-w-[80px] xs:max-w-[100px] sm:max-w-none">{user.name}</span>
                {user.provider === "google" && (
                  <span className="hidden sm:inline text-[10px] bg-blue-100 dark:bg-blue-900/30 text-blue-600 dark:text-blue-400 px-1.5 py-0.5 rounded-full uppercase tracking-wider font-bold shrink-0">
                    Google
                  </span>
                )}
              </div>
